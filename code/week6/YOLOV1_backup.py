...
class YOLOv1(nn.Module):
    def __init__(self, S=7, B=2, C=20):
        ...
        self.conv_layers = nn.Sequential(
            ...
            # 第四层 (重复4次的部分简化写在一层里了，这里严格按照文档结构复现)
            nn.Conv2d(512, 256, kernel_size=1, stride=1),
            nn.LeakyReLU(0.1),
            nn.Conv2d(256, 512, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(0.1),
            # ... (省略中间重复层，保持文档简洁性，实际应用可增加深度) ...
...
class YoloLoss(nn.Module):
    ...
    def forward(self, predictions, target):
        ...
        # 分离目标部分 (Target只包含一个真实框，B=1的结构)
        # 注意：这里需要根据Dataset构建方式调整，假设target前5位是bbox信息
        target_boxes = target[..., :5].view(-1, self.S, self.S, 1, 5) 
        target_classes = target[..., 5:] # 假设5之后是类别
        ...
        target_conf_obj = target_boxes_expanded[..., 4][resp_mask]
...
class VOCDataset(Dataset):
    ...
    def parse_voc_annotation(self, target):
        ...
        # 初始化 label: (S, S, 5 + C) -> 注意这里是单目标target构建，不是(S,S,B*5+C)
        # 因为真实标签每个格子只有一个（如果有物体），训练时会广播
        # 文档 Source 960 说是 5*B+C，但通常 Dataset 返回 5+C 或者直接构建好 B*5+C
        # 这里为了配合 Loss 函数的 split，我们构建完整的 (S, S, B*5 + C)
        # 但实际上只有一组真实数据，另一组是占位符
        yolo_target = torch.zeros((self.S, self.S, 5 + self.C)) 
        ...
        for obj in objects:
            ...
            if yolo_target[grid_y, grid_x, 4] == 0:
                ...
                # 5-24: class one-hot
                yolo_target[grid_y, grid_x, 5 + class_idx] = 1
...
if __name__ == "__main__":
    ...
    if use_dummy_data:
        ...
        # 生成随机数据模拟图片 (Batch, 3, 448, 448) 和 标签 (Batch, 7, 7, 25)
        # 标签最后维度 25 = 5 (bbox) + 20 (classes)
        train_loader = [
            (torch.randn(BATCH_SIZE, 3, 448, 448), torch.zeros(BATCH_SIZE, 7, 7, 25))
            for _ in range(10)
        ]
        # 手动给dummy label造一点物体，防止 Loss 全是 0
        for _, target in train_loader:
            target[:, 3, 3, 4] = 1 # 在中间网格放置物体
            target[:, 3, 3, 2:4] = 0.5 # 宽高
            target[:, 3, 3, 5] = 1 # 属于第0类
    else:
