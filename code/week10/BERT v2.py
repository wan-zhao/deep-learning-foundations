...
class BertPretrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        input_ids_masked = pad_or_truncate(
            input_ids_masked,
            pad_value=token2id['[PAD]'],
            max_len=self.max_len
        )
        mlm_labels = pad_or_truncate(mlm_labels, pad_value=-100, max_len=self.max_len)
        attention_mask = [1 if tok != token2id['[PAD]'] else 0 for tok in input_ids_masked]
        return {
...
dataloader = DataLoader(BertPretrainDataset(sentence_pairs, basic_tokenizer), batch_size=4, shuffle=True)
