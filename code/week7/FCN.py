...
class VOCSegmentationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.image_set == 'train':
            ...
            target_size = int(256 * resize_scale) # 基础尺寸建议改为 480 或 512
...
root = 'C:\TZ\Code\week7\VOCdevkit\VOC2007' 
...
def train_fcn(model, dataloader, epochs, lr, device):
    ...
    scheduler = torch.optim.lr_scheduler.PolyLR(optimizer, power=0.9, max_iter=epochs)
    criterion = nn.CrossEntropyLoss(ignore_index=255) # 忽略无效类别值
...
train_fcn(model, train_loader, epochs=20, lr=1e-4, device=device)
