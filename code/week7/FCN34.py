...
root = r'C:\TZ\Code\week7\VOCdevkit\VOC2007' 
...
def train_fcn(model, dataloader, epochs, lr, device):
    ...
    # 【优先级1】设置类别权重来解决类别不平衡问题
    # 背景类(0)的权重设为 0.1，其他物体类别权重设为 1.0
    weights = torch.ones(21).to(device)
    weights[0] = 0.1  # 大幅降低背景(Class 0)的权重
...
train_fcn(model, train_loader, epochs=20, lr=0.01, device=device)  # 修改学习率为 0.01
